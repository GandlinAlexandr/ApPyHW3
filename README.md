...
В некоторых случаях кэш сбрасывается в случае измения таблиц. Например, в случае `GET /links/search`, чтобы сразу было видно, что ссылка удалена или истекала и не висела в ответе эндпоинта, фактически отсутствуя в базе (как, например, в проектах - я намеренно оставил кэш работать в них).
...
- [x] Дополнительные функции:
  ...
  - [x] Отображение истории всех истекших ссылок с информацией о них, причём для авторизованнаго пользователя.
